...
export const ProductListItem = ({ id, title, category, description, image, createdAt, price, refreshProducts, setIsLoading }: IProductsListItemProps) => {
  ...
  return (
    <>
      ...
      <tr key={id}>
        <td className='px-6 py-4 whitespace-nowrap truncate hover:bg-gray-100 border-grey-light border'>
          <div className='flex items-center'>
            ...
            <div className='ml-4'>
              ...
              <div className='text-xs text-gray-500'>Fecha: { normalizeDate(createdAt) }</div>
              <div className='text-xs text-white bg-purple-400 rounded-md inline-block px-2 py-1'>{ category.title }</div>
            </div>
          </div>
        </td>

        <td className='border-grey-light border hover:bg-gray-100 p-3 max-w-xs truncate'>
          { description }
        </td>

        <td className='border-grey-light border hover:bg-gray-100 p-3 whitespace-nowrap truncate text-center'>
        ...
        <td className='border-grey-light border p-3 hover:font-medium'>
          ...
          <Button
            ...
            classes='w-full text-sm mt-4 border-2 border-solid border-gray-900 text-gray-900 font-bold'
          >
            Borrar
          </Button>
        </td>
